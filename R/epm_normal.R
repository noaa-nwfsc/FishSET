epm_normal <- function(starts3, dat, otherdat, alts, project, expname, mod.name) {
  ...
  #'
  #' @param starts3 Starting values as a numeric vector. The order of parameters in the vector is: \cr
  ...
  #' @param dat Data matrix alternatives with distance.
  #' @param otherdat List that contains other data used in the model, see section 8.4.3 in the 
  ...
  #   (sigma_epsilon in user manual). If length of values before scale parameter is not equal to the
  #   number of alts, use the first param for standard deviation
  ...
  # SUM(beta_jm * G_im), which is the expected mean of the catch function
  ...
  # calculmean of normal function
  empgridmu <- .rowSums(empgridbetas[, cj, ] * empgriddat[, 1, ], obsnum, gridnum) # note grid data same across all alternatives
  
  # log-likelihood for normal function
  ldcatch <- (matrix((-(0.5) * log(2 * pi)), obsnum)) + (-(0.5) * log(matrix(empstdev, obsnum)^2)) + 
    (-(0.5) * (((yj - empgridmu) / (matrix(empstdev, obsnum)))^2))
