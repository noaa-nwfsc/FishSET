...
bycatch <- function(dat, project, cpue, catch = NULL , date, names = NULL, group = NULL, 
  ...
  if (is.null(group)) {
    ...
    effort <- stats::aggregate(dataset[cpue], 
  ...
  if (is.null(group)) {
    ...
    effort <- reshape2::melt(effort, id.vars = c("Year", period), 
                             variable.name = "species_cpue", value.name = "avg_cpue")  
    
  } else if (!is.null(group) & length(group) == 1) {
    
    effort <- reshape2::melt(effort, id.vars = c("Year", period, group1), 
                             variable.name = "species_cpue", value.name = "avg_cpue")  
    
  } else if (!is.null(group) & length(group) > 1) {
    
    effort <- reshape2::melt(effort, id.vars = c("Year", period, group1, group2), 
                             variable.name = "species_cpue", value.name = "avg_cpue")  
  }
  ...
  if (is.null(group)) {
    ...
    count <- stats::aggregate(dataset[catch], 
                              by = list(dataset$Year, 
                                        format(date_parser(dataset[[date]]), p)), 
                              FUN = sum)
  ...
  if (is.null(group)) {
    ...
    count <- reshape2::melt(count, id.vars = c("Year", period), 
                            variable.name = "species_count", value.name = "total")  
    
  } else if (!is.null(group) & length(group) == 1) {
    
    count <- reshape2::melt(count, id.vars = c("Year", period, group1), 
                            variable.name = "species_count", value.name = "total")  
    
  } else if (!is.null(group) & length(group) > 1) {
    
    count <- reshape2::melt(count, id.vars = c("Year", period, group1, group2), 
                            variable.name = "species_count", value.name = "total")  
  }
  ...
  if (value == "stc") {
    ...
    if (is.null(group)) {
      ...
      count <- stats::aggregate(stats::reformulate(paste("Year +", period), "total"), 
      ...
      count <- stats::aggregate(stats::reformulate(paste("Year +", period, "+", group1), "total"), 
                                data = count, FUN = function(x) x/sum(x))
      ...
      count <- stats::aggregate(stats::reformulate(paste("Year +", period, "+", group1, "+", group2), "total"), 
                                data = count, FUN = function(x) x/sum(x))
  ...
  if (is.null(group)) {
    ...
    bycatch <- merge(effort, count, by = c("Year", period, "species_count"),
                     all = TRUE)
    
  } else if (!is.null(group) & length(group) == 1) {
    
    bycatch <- merge(effort, count, by = c("Year", period, group1, "species_count"), 
                     all = TRUE)
    
  } else if (!is.null(group) & length(group) > 1) {
    
    bycatch <- merge(effort, count, by = c("Year", period, group1, group2, "species_count"), 
                     all = TRUE)
  }
  ...
  bycatch <- reshape2::melt(bycatch,measure.vars = c("avg_cpue", if (value == "stc") "stc" else "total"), 
  ...
  bycatch_function$args <- c(dat, project, cpue, catch, date, names, group, year, 
                             period, value, output, format_tab)
  ...
  if (output == "plot") {
    ...
    if (format_tab == "wide") {
      ...
      bycatch <- reshape2::dcast(bycatch, ... ~ species + measure, fun.aggregate = sum, 
