...
#' @examples
#' \dontrun{
#' cpue(pollockMainDataTable, "myproject",
#'   xWeight = "f1Weight",
#'   xTime = "Hour", "f1_cpue"
#' )
#'
...
bycatch <- function(dat, project, cpue, catch, date, period = "year", names = NULL,
  ...
  if (length(cpue) > 1) {
    effort <- reshape2::melt(effort,
      measure.vars = cpue, variable.name = "species_cpue",
  ...
  if (length(catch) > 1) {
    count <- reshape2::melt(count,
      measure.vars = catch, variable.name = "species_count",
      value.name = "catch"
    )
  }
  # STC ====
  ...
  catch_plots <- lapply(names, function(x) {
    ...
    ca_plot <- ggplot2::ggplot(catch_dat, ggplot2::aes_string(period, f_catch2(), group = f_group(), color = group1)) +
      ...
      ggplot2::geom_line(size = 0.65) +
      ggplot2::labs(
        title = if (value == "total") "Total" else "STC",
  ...
  if (format_tab == "long") {
    bycatch <- reshape2::melt(bycatch,
      id.vars = c(c_nm, "species"),
  ...
  bycatch_function$args <- list(
    dat, project, cpue, catch, date, period, names, group,
