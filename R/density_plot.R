...
#' filter_value = list(2011, 9:11), tran = "log", facet_date = TRUE, 
#' group = "GEAR_TYPE")
#' }
#' @import ggplot2
...
density_plot <- function(dat, project, var, type = "kde", group = NULL, date = NULL, 
  ...
  nm <- unique(c(date, var, group[!(group %in% group_date)], 
  ...
  if (end == FALSE) {
    ...
    density_plot_function$args <- list(dat, project, var, type, group, date, filter_value, 
