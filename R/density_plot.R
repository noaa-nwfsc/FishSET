...
density_plot <- function(dat, project, var, type = c("kde", "ecdf", "cdf"), group = NULL, date = NULL, facet_year = FALSE, year = NULL, position = c("identity", 
    "stack", "fill")) {
    ...
    if (type == "kde") {
        ...
        plot <- ggplot2::ggplot(dataset, ggplot2::aes_string(x = var, fill = if (is.null(group) == F) {
            ...
            NULL
        })) + ggplot2::stat_density(position = position, color = "black", alpha = 0.7) + ggplot2::labs(title = paste("KDE of", var, if (!is.null(year) & 
            length(year) == 1) 
            year else NULL)) + fishset_theme + if (facet_year == TRUE | length(year) > 1) {
        ...
        if (is.null(group)) {
            ...
            plot <- ggplot2::ggplot(dataset, ggplot2::aes_string(x = var)) + ggplot2::stat_ecdf(geom = "area", alpha = 0.7) + ggplot2::stat_ecdf(geom = "step", 
                alpha = 0.7) + ggplot2::labs(title = paste("ECDF of", var, if (!is.null(year) & length(year) == 1) 
                year else NULL)) + fishset_theme + if (facet_year == TRUE | length(year) > 1) {
                ggplot2::facet_wrap(~Year)
            } else {
                ggplot2::geom_blank()
            ...
            plot <- ggplot2::ggplot(dataset, ggplot2::aes_string(x = var, color = group)) + ggplot2::stat_ecdf(geom = "step") + ggplot2::labs(title = paste("ECDF of", 
                var, if (!is.null(year) & length(year) == 1) 
                  year else NULL)) + fishset_theme + if (facet_year == TRUE | length(year) > 1) {
                ggplot2::facet_wrap(~Year)
            } else {
                ggplot2::geom_blank()
        ...
        dataset$cdf <- stats::pnorm(dataset[[var]], mean = mean(dataset[[var]]), sd = sd(dataset[[var]]))
        
        plot <- ggplot2::ggplot(dataset, ggplot2::aes_string(var)) + ggplot2::geom_area(ggplot2::aes(y = cdf), position = "identity", alpha = 0.7) + ggplot2::labs(title = paste("KDE of", 
            var, if (!is.null(year) & length(year) == 1) 
                year else NULL)) + fishset_theme + if (facet_year == TRUE | length(year) > 1) {
            ggplot2::facet_wrap(~Year)
        } else {
