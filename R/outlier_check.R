...
outlier_table <- function(dataset, x) {
    ...
    # flog_func(dataset=df.name, x=x.name, fun.name='outlier_table') Check if data is numeric. Cannot check outliers if not.
    if (is.numeric(dataset[, x]) == T) {
        # Output table of summary statistics Row 1 No data removed
        dat.table <- data.frame(Vector = x.name, outlier_check = "None", N = length(dataset[, x]), mean = mean(dataset[, x], na.rm = T), median = median(dataset[, 
            x], na.rm = T), SD = sd(dataset[, x]), min = min(dataset[, x], na.rm = T), max = max(dataset[, x], na.rm = T), NAs = sum(length(which(is.na(dataset[, 
            x])))), skew = skewness(dataset[, x]))
        # Row 2 5-95% quantile
        ...
        dat.table <- rbind(dat.table, data.frame(Vector = x.name, outlier_check = "5_95_quant", N = length(temp[, x]), mean = mean(temp[, x], na.rm = T), 
            median = median(temp[, x], na.rm = T), SD = sd(temp[, x]), min = min(temp[, x], na.rm = T), max = max(temp[, x], na.rm = T), NAs = sum(length(which(is.na(temp[, 
                x])))), skew = skewness(temp[, x])))
        ...
        dat.table <- rbind(dat.table, data.frame(Vector = x.name, outlier_check = "25_75_quant", N = length(temp[, x]), mean = mean(temp[, x], na.rm = T), 
            median = median(temp[, x], na.rm = T), SD = sd(temp[, x]), min = min(temp[, x], na.rm = T), max = max(temp[, x], na.rm = T), NAs = sum(length(which(is.na(temp[, 
                x])))), skew = skewness(temp[, x])))
        # Row 4 Mean +/2SD
        temp <- dataset[dataset[, x] < (mean(dataset[, x], na.rm = T) + 2 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (mean(dataset[, x], na.rm = T) - 
            2 * sd(dataset[, x], na.rm = T)), ]
        dat.table <- rbind(dat.table, data.frame(Vector = x.name, outlier_check = "mean_2SD", N = length(temp[, x]), mean = mean(temp[, x], na.rm = T), 
            median = median(temp[, x], na.rm = T), SD = sd(temp[, x]), min = min(temp[, x], na.rm = T), max = max(temp[, x], na.rm = T), NAs = sum(length(which(is.na(temp[, 
                x])))), skew = skewness(temp[, x])))
        # Row 5 Mean +/3SD
        temp <- dataset[dataset[, x] < (mean(dataset[, x], na.rm = T) + 3 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (mean(dataset[, x], na.rm = T) - 
            3 * sd(dataset[, x], na.rm = T)), ]
        dat.table <- rbind(dat.table, data.frame(Vector = x.name, outlier_check = "mean_3SD", N = length(temp[, x]), mean = mean(temp[, x], na.rm = T), 
            median = median(temp[, x], na.rm = T), SD = sd(temp[, x]), min = min(temp[, x], na.rm = T), max = max(temp[, x], na.rm = T), NAs = sum(length(which(is.na(temp[, 
                x])))), skew = skewness(temp[, x])))
        # Row 6 Median +/-2SD
        temp <- dataset[dataset[, x] < (median(dataset[, x], na.rm = T) + 2 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (median(dataset[, x], na.rm = T) - 
            2 * sd(dataset[, x], na.rm = T)), ]
        dat.table <- rbind(dat.table, data.frame(Vector = x.name, outlier_check = "median_2SD", N = length(temp[, x]), mean = mean(temp[, x], na.rm = T), 
            median = median(temp[, x], na.rm = T), SD = sd(temp[, x]), min = min(temp[, x], na.rm = T), max = max(temp[, x], na.rm = T), NAs = sum(length(which(is.na(temp[, 
                x])))), skew = skewness(temp[, x])))
        # Row 7 Median +/-3SD
        temp <- dataset[dataset[, x] < (median(dataset[, x], na.rm = T) + 3 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (median(dataset[, x], na.rm = T) - 
            3 * sd(dataset[, x], na.rm = T)), ]
...
outlier_plot <- function(dataset, x, outlier.mod = "none", x.dist = "normal", save.output = TRUE, remove = F) {
    # logging function information df.name <- deparse(substitute(dataset)) x.name <- deparse(substitute(x)) flog_func(dataset=df.name, x=x.name,
    # fun.name='outlier_plot') Check if data is numeric. Cannot check outliers if not.
    if (is.numeric(dataset[, x]) == T) {
        # Begin outlier check
        ...
        if (outlier.mod == "none") {
            ...
            if (outlier.mod == "5_95_quant") {
                ...
                dat_sub <- dataset[dataset[, x] < (mean(dataset[, x], na.rm = T) + 2 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (mean(dataset[, x], 
                  na.rm = T) - 2 * sd(dataset[, x], na.rm = T)), ]
            } else if (outlier.mod == "median_2SD") {
                dat_sub <- dataset[dataset[, x] < (median(dataset[, x], na.rm = T) + 2 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (median(dataset[, 
                  x], na.rm = T) - 2 * sd(dataset[, x], na.rm = T)), ]
            } else if (outlier.mod == "mean_3SD") {
                dat_sub <- dataset[dataset[, x] < (mean(dataset[, x], na.rm = T) + 3 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (mean(dataset[, x], 
                  na.rm = T) - 3 * sd(dataset[, x], na.rm = T)), ]
            } else if (outlier.mod == "median_3SD") {
                dat_sub <- dataset[dataset[, x] < (median(dataset[, x], na.rm = T) + 3 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (median(dataset[, 
                  x], na.rm = T) - 3 * sd(dataset[, x], na.rm = T)), ]
            }
        }  #End Outlier mod
        # open a pdf file
        if (save.output == "TRUE") {
...
outlier_remove <- function(dataset, x, outlier.mod = "none", remove = T) {
    # logging function information df.name <- deparse(substitute(dataset)) x.name <- deparse(substitute(x)) flog_func(dataset=df.name, x=x.name,
    # fun.name='outlier_remove') Check if data is numeric. Cannot check outliers if not.
    if (is.numeric(dataset[, x]) == T) {
        # Begin outlier check
        if (remove == TRUE) 
            {
                # log actions
                write_lines(layout.json.ed(trace, "outlier_remove", dataset, x, msg = paste("outliers removed using", outlier.mod)), paste(getwd(), "/Logs/", 
                  Sys.Date(), ".json", sep = ""), append = T)
                # flog.trace('Outliers checked. Outliers deemed present in %s of dataframe %s. Values outside %s removed.', x.name, df.name, outlier.mod) Remove
                # outliters
                if (outlier.mod == "none") {
                  dataset <- dataset
                  ...
                  dataset <- dataset[dataset[, x] < (mean(dataset[, x], na.rm = T) + 2 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (mean(dataset[, x], 
                    na.rm = T) - 2 * sd(dataset[, x], na.rm = T)), ]
                } else if (outlier.mod == "median_2SD") {
                  dataset <- dataset[dataset[, x] < (median(dataset[, x], na.rm = T) + 2 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (median(dataset[, 
                    x], na.rm = T) - 2 * sd(dataset[, x], na.rm = T)), ]
                } else if (outlier.mod == "mean_3SD") {
                  dataset <- dataset[dataset[, x] < (mean(dataset[, x], na.rm = T) + 3 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (mean(dataset[, x], 
                    na.rm = T) - 3 * sd(dataset[, x], na.rm = T)), ]
                } else if (outlier.mod == "median_3SD") {
                  dataset <- dataset[dataset[, x] < (median(dataset[, x], na.rm = T) + 3 * sd(dataset[, x], na.rm = T)) & dataset[, x] > (median(dataset[, 
                    x], na.rm = T) - 3 * sd(dataset[, x], na.rm = T)), ]
                }
                return(dataset)
            }  #End Outlier check
