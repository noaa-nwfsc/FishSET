...
outlier_table <- function(dat, project, x) {
    ...
    filledrow <- function(dat, x) {
        data.frame(N = length(temp[, x]), mean = round(mean(temp[, x], na.rm = T), 2), median = round(stats::median(temp[, x], na.rm = T), 2), SD = round(stats::sd(temp[, 
            x], na.rm = T), 2), min = round(min(temp[, x], na.rm = T), 2), max = round(max(temp[, x], na.rm = T), 2), NAs = sum(length(which(is.na(temp[, 
            x])))), skew = round(skewness(temp[, x], na.rm = T), 2))
    }
    ...
    if (is.numeric(dataset[, x]) == T) {
        ...
        dat.table <- data.frame(Vector = x.name, outlier_check = "None", N = length(dataset[, x]), mean = round(mean(dataset[, x], na.rm = T), 2), median = round(stats::median(dataset[, 
            x], na.rm = T), 2), SD = round(sd(dataset[, x], na.rm = T), 2), min = round(min(dataset[, x], na.rm = T), 2), max = round(max(dataset[, x], 
            na.rm = T), 2), NAs = sum(length(which(is.na(dataset[, x])))), skew = round(skewness(dataset[, x], na.rm = T), 2))
        # Row 2 5-95% quantile
...
outlier_plot <- function(dat, project, x, dat.remove, x.dist, output.screen = FALSE) {
    ...
    if (is.numeric(dataset[, x]) == T) {
        ...
        mytheme <- ggplot2::theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"), 
            axis.text = element_text(size = 9), axis.title = element_text(size = 8))
        ## Plot 1!
        distplot <- function(gdf, first, second) {
            ggplot2::ggplot(gdf, ggplot2::aes_string(x = first, y = second)) + ggplot2::geom_point(color = "red", na.rm = TRUE) + ggplot2::geom_point(data = dat_sub, 
                ggplot2::aes_string(x = first, y = second), color = "blue", na.rm = TRUE) + ggplot2::labs(x = "Data row") + mytheme
        }
        ...
        if (x.dist == "normal") {
            ...
            p2 <- p2 + ggplot2::stat_function(fun = dweibull, colour = "blue", args = list(shape = 1.2/sqrt(var(log(dat_sub[, x]), na.rm = TRUE)), scale = mean(dat_sub[, 
                x], na.rm = TRUE) + 0.572/(1.2/sqrt(var(log(dat_sub[, x]), na.rm = TRUE)))))
