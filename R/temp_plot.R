 ...
 #Date var - NULL or use the first one
  if(is.null(date.var)){
    ...
    if(len.fun=='length'){
      ...
      if(length(unique(lubridate::year(date_parser(dataset[[date.y.var]]))))>1){
        df2l <-aggregate(dataset[[var.select]] ~ lubridate::year(date_parser(dataset[[date.y.var]])),FUN=function(x) round(length(x)/nrow(dataset)*100,2))
      } else {
        df2l <-aggregate(dataset[[var.select]] ~lubridate::month(date_parser(dataset[[date.y.var]])),FUN=function(x) round(length(x)/nrow(dataset)*100,2))
      }
    }
...
if(grepl('date', var.select, ignore.case=T)==TRUE){
  p1 <- ggplot2::ggplot(dataset, 
                        ggplot2::aes_string(x=as.Date(dataset[[date.y.var]], origin='01-01-1970'),
                                            y=as.Date(dataset[[var.select]], origin='01-01-1970'))) + ggplot2::geom_point()+
  ...
  p1 <- ggplot2::ggplot(dataset, 
                        ggplot2::aes_string(x=as.Date(dataset[[date.y.var]], origin='01-01-1970'), y=var.select)) +
                        ggplot2::geom_point()+
                        ggplot2::labs(subtitle=paste(var.select, 'by Date'), x="Date", y=var.select) +
                        ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), 
...
p2 <- ggplot2::ggplot(df2l, ggplot2::aes_string(x=df2l[,1], y=df2l[,2]))+ ggplot2::geom_bar(stat='identity')+
      ggplot2::labs(subtitle=paste(simpleCap(len.fun), 'by', tolower(t2)), x=t2, y='') +
      ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), 
                    panel.background = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black"), 
                    axis.text=ggplot2::element_text(size=11), axis.title=ggplot2::element_text(size=11))
if(!is.numeric(dataset[[var.select]])) {
  ...
  p3 <- ggplot2::ggplot(df2m, ggplot2::aes_string(x=df2m[,1], y=df2m[,2])) + 
        ...
        ggplot2::labs(subtitle=paste(simpleCap(agg.fun), 'of value by', tolower(t2)), x=t2, y='')+
        ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), 
                   panel.background = ggplot2::element_blank(), axis.line = ggplot2::element_line(colour = "black"), 
                   axis.text=ggplot2::element_text(size=11), axis.title=ggplot2::element_text(size=11))
} 
