...
species_catch <- function(dat, project, species, date, period = "month", fun = "sum",
  ...
  periods <- c(
    "year", "month", "weeks", "weekday", "weekday_abv", "weekday_num",
    "day", "day_of_year"
  )
  ...
  if (type == "bar") {
    ...
    s_plot <- s_plot + ggplot2::geom_line(ggplot2::aes_string(
      group = f_group1(),
      color = f_group1(),
      linetype = f_group2()
    )) +
  ...
  species_catch_function$args <- list(
    dat, project, species, date, period, fun, group,
