...
create_dist_between_for_gui <- function(dat, start, end, units, name = 'DistBetwen', portTable = NULL, gridfile = NULL, 
    ...
    if (start[1] == end[1]) {
        ...
        if (grepl("centroid", start[1], ignore.case = TRUE) | grepl("centroid", end[1], ignore.case = TRUE)) {
            dat2 <- assignment_column(dat = dataset, gridfile = gridfile, hull.polygon = FALSE, lon.grid = lon.grid, lat.grid = lat.grid, lon.dat = lon.dat, 
                lat.dat = lat.dat, cat = cat, closest.pt = TRUE)
            int <- find_centroid(dat2, gridfile = gridfile, lon.grid == lon.grid, lat.grid == lat.grid, lon.dat = lon.dat, lat.dat = lat.dat, cat = cat, 
                weight.var = NULL)
        }
