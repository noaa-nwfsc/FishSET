...
dataVerification <- function(dataset, dataindex){
     ...
     allNecFields=c('name','units','general','XY','ID','Time','Catch','Effort','CPUE','Lat','Value','Area','Port','Price','Trip','Haul','Other')
     ...
     unitsAvailable <- c('Day of Month','Day of Year','Decimal Degree','Dollar', 'Fathom', 'Feet', 'Horsepower', 'ID', 'Kilometer', 'Pound', 'lbs',
                         'Meter', 'Metric Ton', 'Mile', 'Minute', 'mm','mmdd', 'N/A', 'Numeric', 'Percent', 'Tonne', 'Week', 'wk','Y/N',
                         'T/F', 'yyyymmdd',  'yyyy' , 'yyyy-mm-dd HH:MM:SS', 'mm-dd-yyyy HH:MM:SS', 'mm/dd/yyyy HH:MM:SS', 'yyyy/mm/dd HH:MM:SS', 'min')
     indx <- sapply(unitsAvailable, grepl, colnames(dataindex), ignore.case=TRUE) %>% colSums()
     if(length(which(colSums(sapply(unitsAvailable, grepl, colnames(dataindex)))==0))){
          print('All units are specified.')
     } else {
          print(paste('The units are not recognized for the following variables:',
