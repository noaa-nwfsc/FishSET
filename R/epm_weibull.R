epm_weibull <- function(starts3, dat, otherdat, alts, project, expname, mod.name) {
    ...
    # if number of parameters before scale parameter is not equal to number of alts, use first parameter as sigma catch
    ...
    ldcatch <- (matrix((log(empk)), obsnum)) + (matrix((-(empk)), obsnum) * log(empgridmu)) + (matrix((empk - 1), obsnum) * log(yj)) + (-((yj/empgridmu)^(matrix(empk, 
        obsnum))))
