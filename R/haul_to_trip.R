...
#  }
  ...
  drop <- 0
  if (length(grep("DUR", names(int[, c(which(as.data.frame(dataIndex[
    dataIndex[, "variable_name"] == colnames(int[, -which(colnames(int) == "rowID")]),
    "generalType"
  ]) == "Time"))]), ignore.case = T)) != 0) {
    drop <- grep("DUR", names(int[, c(which(as.data.frame(dataIndex[
      dataIndex[, "variable_name"] == colnames(int[, -which(colnames(int) == "rowID")]),
      "generalType"
    ]) == "Time"))]), ignore.case = T)
  }
  ...
  # Time - duration
  if (length(which(as.data.frame(dataIndex[dataIndex[, "variable_name"] %in% colnames(int[, -which(colnames(int) == "rowID")]), "generalType"]) == "Time")) >
    0 & any(grepl("dur", colnames(int)[which(as.data.frame(dataIndex[
    dataIndex[, "variable_name"] %in% colnames(int[, -which(colnames(int) == "rowID")]),
    "generalType"
  ]) == "Time")], ignore.case = TRUE)) == TRUE) {
    out <- cbind(out, suppressWarnings(stats::aggregate(cbind(as.data.frame(int[, c(which(as.data.frame(dataIndex[dataIndex[, "variable_name"] %in%
      ...
      ,
      "variable_name"
    ] %in% colnames(int[, -which(colnames(int) == "rowID")]), "generalType"]) == "Time"))]), ignore.case = T, invert = TRUE)]),
    rowID = int$rowID
    ), list(int$rowID), match.fun(fun.numeric), na.rm = TRUE))[, -1])
  }
  # Other numeric
