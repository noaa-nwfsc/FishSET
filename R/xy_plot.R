xy_plot <- function(dat, project, var1, var2, regress = FALSE) {
    ...
    if (regress == FALSE) {
        plot <- ggplot2::ggplot(dataset, ggplot2::aes_string(x = dataset[[var1]], y = dataset[[var2]])) + ggplot2::geom_point() + ggplot2::labs(subtitle = paste(var1, 
            "by", var2), x = var1, y = var2) + ggplot2::theme(panel.grid.major = ggplot2::element_blank(), panel.grid.minor = ggplot2::element_blank(), 
        ...
        plot <- ggpubr::annotate_figure(ggpubr::ggarrange(ggplot2::ggplot(dataset, ggplot2::aes_string(x = dataset[[var1]], y = dataset[[var2]])) + ggplot2::geom_point() + 
            ggplot2::geom_smooth(method = lm) + ggplot2::labs(subtitle = paste(var1, "against", var2), x = var1, y = var2) + ggplot2::theme(panel.grid.major = ggplot2::element_blank(), 
            ...
            axis.text = ggplot2::element_text(size = 11), axis.title = ggplot2::element_text(size = 11)), ggplot2::ggplot(lm(dataset[[var1]] ~ dataset[[var2]])) + 
            ggplot2::geom_point(ggplot2::aes(x = .fitted, y = .resid)) + ggplot2::labs(subtitle = "Residuals against fitted values", x = "Fitted", y = "Residuals") + 
