moran_stats <- function(dat, project, varofint, gridfile, lon.dat=NULL, lat.dat=NULL, cat=NULL, lon.grid=NULL, lat.grid=NULL) {
  ...
  #4. Identify variable of interest
  #5.
  ...
  nb.rk <- spdep::knn2nb(spdep::knearneigh(as.matrix(uniquedatatomap[,c("centroid_lon",
  ...
  moranmap <- ggplot(data=datatomap) +
    ...
    annotate(geom='text', x = min(datatomap$path_lon)*0.9915, 
             y = min(datatomap$path_lat)*0.997,
             label=paste0("Global Moran's I = ", round(gmoranspdep$estimate[1],2)),
             parse=FALSE, size = annotatesize, color = "black", hjust = 0) +
    annotate(geom='text', x = min(datatomap$path_lon)*0.9915, 
             y = min(datatomap$path_lat)*0.994,
