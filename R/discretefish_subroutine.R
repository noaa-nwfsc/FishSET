...
discretefish_subroutine <-
  ...
  if (end == FALSE) {
    ...
    for (i in 1:length(x_temp)) { # loop thru each model
     ...
     # TODO: fix this loop. "individual" or custom may have length of 1 or more, 
     # "all" will always have length of 1
      ...
      # data matrix ----
      # loop thru expected catch matrices
      for (j in seq_len(length.exp.names)) {
        
        # Note: this will not follow what user specified in model design (expectecatchmodels arg)
        # TODO: add expectcatchmodels arg to create_model_input()
        # if (length.exp.names == 1) {
          
          datamatrix <- create_model_input(project = project, x = x, mod.name = x$mod.name, 
        ...
        #   datamatrix <- create_model_input(project = project, x = x, mod.name = x$mod.name, 
        #                                    use.scalers = use.scalers, scaler.func = scaler.func, 
        #                                    expected.catch = x$gridVaryingVariables[j],
        #                                    exp.names = x$expectcatchmodels)  
        # }
