...
#' @param initparams Vector, initial parameter estimates for revenue/location-specific covariates then cost/distance. The number of parameter estimate varies by likelifood function. See Details section for more information.
#' @param optimOpt  String, optimization options [max function evaluations, max iterations, (reltol) tolerance of x, trace].
#' @param methodname String, optimization method (see \code{\link[stats]{optim}} options).
...
#' @import shiny
#' @details 
#' Runs through model design choices generated by \code{\link{make_model_design}} and stored as ‘modelInputData’ in FishSET database. 
#' Required data, optional data, and details on likelihood functions are outlined in \code{\link{make_model_design}}. \cr
#' Likelihood-specific initial parameter estimates:
#'  \tabular{rlll}{
#'    logit_c: \tab Conditional logit likelihood \cr
#'    Starting parameter values takes the order of: c([alternative-specific parameters],  [travel-distance parameters]). \cr \cr 
...
#'    Starting parameters takes the order of: c([average-catch parameters], [travel-distance parameters]). \cr \cr 
...
#'    Starting parameter values takes the order of: c([marginal utility from catch], [catch-function parameters], [polynomial starting parameters], [travel-distance parameters], [catch sigma]). \cr \cr 
#'    The number of polynomial interaction terms is currently set to 2, so given the chosen degree 'polyn' there should be 
#'    (((polyn+1)*2) + 2)*(k) polynomial starting parameters, where (k) equals the number of alternative fishing choices. The marginal utility 
#'    from catch and catch sigma are of length equal to unity respectively. The catch-function and travel-distance parameters are of length (# of
#'    catch variables)*(k) and (# of cost variables) respectively.}
#'  
#'    epm_normal: \tab Expected profit model with normal catch function \cr 
#'    Starting parameters values takes the order of: c([catch-function parameters], [travel-distance parameters], [catch sigma(s)], [scale parameter]). \cr \cr
...
#'    Starting parameter values takes the order of: c([catch-function parameters], [travel-distance parameters], [catch sigma(s)], [scale parameter]). \cr \cr
...
#'    Starting parameter values takes the order of: c([catch-function parameters], [travel-distance
...
#'    \tabular{rlll}{
#'    \code{\link{model_out_view}} \tab model output including optimization information, standard errors, coefficients, and t- statistics. 
