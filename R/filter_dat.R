filter_table <- function(dat, project, x, exp) {
    ...
    #' The filter table will be saved in the FishSET database under the \code{project} name and (\emph{filterTable}). 
...
filter_dat <- function(dat, exp, filterTable) {
    #' Remove rows based on filter expressions defined in \code{\link{filterTable}}.
    #'
    #' @param dat Primary data containing information on hauls or trips. Table in FishSET database contains the string 'MainDataTable'.
