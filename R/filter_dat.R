filter_table <- function(dat, project, x, exp) {
  ...
  #' @param exp Filter expression. Should take on the form of \code{"x < 100"} or 
  #'   \code{"is.na(x) == FALSE"}.
  ...
  #'   the FishSET database under the \code{project} name and `filterTable`. The 
  #'   new filter functions are added each time the function is run and the table 
  #'   is automatically updated in the FishSET database. The function call will be 
...
filter_dat <- function(dat, project, exp, filterTable = NULL) {
  ...
  #' @param exp How to filter. May be a row in the filter table generated by 
  #'   \code{\link{filter_table}} that contains a filter expression or the filter 
  #'   expression to apply to the data. If the filter expression is supplied, it 
  #'   should take on the form of \code{"x < 100"} or \code{"is.na(x) == FALSE"}.
  #' @param filterTable Name of filter table in FishSET database. Name should 
  #'   contain the phrase 'filterTable'.
  #' @keywords filter, subset
  #' @export filter_dat
  #' @return Filtered data frame
  #' @details Filter data frame based on a predefined filter expression from 
  #'   \code{\link{filter_table}} or a filter expression. We recommend creating 
  #'   a filter table using \code{\link{filter_table}} so that filter expressions 
