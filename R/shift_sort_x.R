shift_sort_x <- function(x, ch, y, distance, alts, ab) {
  ...
  for (j in 1:n) {
    if (ch0[j, ] == 0) {
      ...
      xsorted <- t(as.matrix(x[j, ]))  #need to 'as.matrix' again because subsetting turns into named num (one dim).
      
    } else {
      
      xj <- as.matrix(x[j, ])  #need to 'as.matrix' again because subsetting turns into named num (one dim). I
