...
spatial_hist <- function(dat, group) {
    ...
    ggplot(melt.dat, aes(value, group = group, fill = group)) + geom_histogram(position = "identity", alpha = 0.5, binwidth = 0.25) + facet_wrap(~variable, 
...
spatial_summary <- function(dat, stat.var = c("length", "no_unique_obs", "perc_total", "mean", "median", "min", "max", "sum"), variable, gridfile, lon.grid, 
    lat.grid, lon.dat, lat.dat, cat) {
    ...
    if (stat.var == "no_unique_obs") {
        graphics::plot(aggregate(dataset[[variable]], by = list(dataset[, date.var]), function(x) length(unique(x))), type = "l", lty = 1, ylab = paste("No. unique obs", 
        ...
        plot(aggregate(dataset[[variable]], by = list(dataset[, date.var]), function(x) length((x))/length(dataset[[variable]]) * 100), type = "l", lty = 1, 
