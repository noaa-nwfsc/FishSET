...
# read.csv('Data/adfgstat6_map.csv', header = F) require(rjson) map <-
# fromJSON('C:/Users/Melanie/Documents/FishSET_RPackage/Data/NMFS_RA.json',
# flatten=TRUE) map <- map$features
...
assignment_column <- function(dataset, gridfile, hull.polygon = c(TRUE, FALSE), lon.grid, lat.grid, 
  ...
  my_spatial_polys <- sp::SpatialPolygons(p1, proj4string = CRS("+proj=longlat +datum=WGS84"))
  ...
  sp::proj4string(dat_sub) <- CRS("+proj=longlat +datum=WGS84")  #proj4string(sodo)
