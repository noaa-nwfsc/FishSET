getis_ord_stats <- function(dat, project, varofint, gridfile, lon.dat = NULL, lat.dat = NULL, cat = NULL, lon.grid = NULL, lat.grid = NULL) {
    ...
    if (x == 0) {
        ...
        getismap <- ggplot(data = datatomap) + geom_path(aes(x = path_lon, y = path_lat, group = ZoneID), color = "black", size = 0.375) + geom_map(data = world, 
            map = world, aes(map_id = region), fill = "grey", color = "black", size = 0.375) + geom_polygon(data = datatomap, aes(x = path_lon, y = path_lat, 
            group = ZoneID, fill = GetisOrd), color = "black", alpha = 1, size = 0.375) + xlim(minlon, maxlon) + ylim(minlat, maxlat) + scale_fill_gradient2(low = "skyblue2", 
            high = "firebrick1", mid = "white", name = "Local\nGetis-Ord") + ggtitle("Getis-Ord statistics") + annotate(geom = "text", x = min(datatomap$path_lon) * 
            ...
            color = "black", hjust = 0) + annotate(geom = "text", x = min(datatomap$path_lon) * 0.9915, y = min(datatomap$path_lat) * 0.994, label = paste0("p-value = ", 
