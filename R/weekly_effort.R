...
weekly_effort <- function(dat, project, cpue, date, group = NULL, year = NULL, plot_type = "line_point", 
  ...
  if (length(cpue) == 1) {
    ...
    if (is.null(group)) {
      ...
      effort <- stats::aggregate(dataset[[cpue]], by = list(dataset$years,
                                                            dataset$week),
                                 FUN = mean, drop = TRUE)  
    ...
    if (is.null(group)) {
      ...
      missing_periods <- expand.grid(week = ind,
                                     years = unique(effort$years),
                                     group1 = unique(effort[[group1]]))
    ...
    plot <- ggplot2::ggplot(effort, ggplot2::aes(week, avg_cpue, color = if (length(unique(.data[["years"]])) > 1) years else NULL))
    
    if (plot_type == "line") { 
      
      plot <- plot + ggplot2::geom_line(size = .65)
      ...
      plot <- plot + ggplot2::geom_point() + ggplot2::geom_line(size = .65)
      
    }
    
    plot <- plot + 
      ggplot2::labs(title = if (!is.null(year) & length(year) == 1) paste(year, "average CPUE by week") else "average CPUE by week",
                    y = "average cpue") + 
    ...
    if (length(unique(effort$years)) > 1) {
      ...
      plot <- plot + ggplot2::guides(color = ggplot2::guide_legend(title = "Year", 
    ...
    if (is.null(group)) {
      ...
      plot <- plot + ggplot2::facet_grid(cpue ~ ., scales = "free_y",
                                         labeller = ggplot2::labeller(.cols = label_both))
      
    } else if (!is.null(group) & length(group) == 1) {
      
      plot <- plot + ggplot2::facet_grid(stats::reformulate(".", paste("cpue +", group1)), scales = "free_y",
                                         labeller = ggplot2::labeller(.cols = label_both,
                                                                      .rows = label_both))
      
    } else if (!is.null(group) & length(group) > 1) {
      
      plot <- plot + ggplot2::facet_grid(stats::reformulate(group2, paste("cpue +", group1)), scales = "free_y",
                                         labeller = ggplot2::labeller(.cols = label_both,
                                                                      .rows = label_both))
    }
  ...
  if (output == "plot") {
    ...
    if (length(cpue) == 1) {
      ...
      if (format_tab == "wide") {
        ...
        effort <- reshape2::dcast(effort, ... ~ cpue, value.var = "avg_cpue", 
