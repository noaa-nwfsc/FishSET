...
weekly_effort <- function(dat, project, cpue, date, group = NULL, year = NULL, plot_type = "line", 
                          output = c("plot", "table")){
  ...
  plot <- ggplot2::ggplot(effort, ggplot2::aes(week, avg_cpue, color = if (length(unique(effort$years)) > 1) years else NULL))
  ...
  weekly_effort_function$args <- c(dat, project, cpue, date, group, year, plot_type, output)
