...
weekly_catch <- function(dat, project, species, date, year = NULL, group = NULL, fun = "sum", position = "stack", convert_to_tons = FALSE, 
    ...
    if (is.null(group)) {
        ...
        count <- stats::aggregate(dataset[species], by = list(dataset$Year, dataset$week, dataset[[group1]]), FUN = match.fun(fun))
        
        count <- setNames(count, c("Year", "week", group1, species))
        
    } else {
        
        count <- stats::aggregate(dataset[species], by = list(dataset$Year, dataset$week, dataset[[group1]], dataset[[group2]]), FUN = match.fun(fun))
    ...
    if (is.null(group)) {
        ...
        if (length(species) == 1) {
            ...
            missing_periods <- expand.grid(week = ind, Year = unique(count$Year), species = unique(count$species))
        }
        ...
        if (length(species) == 1) {
            ...
            missing_periods <- expand.grid(week = ind, Year = unique(count$Year), group1 = unique(count[[group1]]))
            
            missing_periods <- setNames(missing_periods, c("week", "Year", group1))
            
        } else {
            
            missing_periods <- expand.grid(week = ind, Year = unique(count$Year), group1 = unique(count[[group1]]), species = unique(count$species))
        ...
        if (length(species) == 1) {
            ...
            missing_periods <- expand.grid(week = ind, Year = unique(count$Year), group1 = unique(count[[group1]]), group2 = unique(count[[group2]]))
    ...
    if (length(species) == 1) {
        ...
        if (!is.null(group) & length(group) == 1) {
            ...
            if (is.null(year) | (!is.null(year) & length(year) == 1)) {
                ...
                plot <- plot + ggplot2::facet_grid(stats::reformulate(group2, paste("Year +", group1)), scales = "free_y", labeller = ggplot2::labeller(.cols = label_both))
            }
