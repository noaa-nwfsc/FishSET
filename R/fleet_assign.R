...
fleet_table <- function(dat, project, cond = NULL, fleet_val = NULL, table = NULL, save = TRUE) {
  ...
  #'
  #' @description \code{fleet_table} saves a table of fleet expression to the FishSET
  #'   database which can then be applied to a dataset with \code{\link{fleet_assign}}. 
  ...
  #'   A table can be created with the \code{cond} and \code{fleet_val} arguments 
  ...
  #'   Must be used with the \code{fleet_val} argument. Each expression should be 
  #'   in quotes (double or single) with nested quotes indicated with escaped quotes 
  #'   (\') or with the opposite quote used to contain the expression. For example, 
  #'   "species == 'cod'" and "species == \'cod\'" are both valid.
  #' @param fleet_val String; a vector of fleet names to be assigned. Must be used 
  #'   with the \code{cond} argument.
  #' @param table A data frame that has one condition column and one fleet column. See
  #'   'Details' for table formatting.
  #' @param save Logical; whether to save the current fleet_table to the FishSET 
  #'   database. Defaults to \code{TRUE}. Tables are saved in the format of 
  #'   'projectFleetTable'. Each project can only have one fleet table. New fleet 
  #'   definitions are appended to the exiting fleet table. See \code{\link{table_remove}} 
  #'   to delete a table.
  #' @importFrom DBI dbConnect dbWriteTable dbDisconnect
  #' @importFrom RSQLite SQLite
  #' @importFrom stats setNames
  #' @export fleet_table
  #' @return Returns a table of fleet conditions that is saved to the FishSET database 
  #'   with the name 'projectFleetTable'.
  #' @details
  ...
  #'   in the condition column example could also be \code{"GEAR == 8 & species == 'pollock'"}. 
  ...
  if (end == FALSE) {
    ...
    log_call(project, fleet_table_function)
...
fleet_assign <- function(dat, project, fleet_tab, assign = NULL, overlap = FALSE, 
                         format_var = "string") {
  ...
  #'   FishSET database contains the string 'MainDataTable'.
  #' @param project String, name of project.
  #' @param fleet_tab String, name of the fleet table stored in FishSET database. 
  ...
  #'   will be used.  
  #' @param overlap Logical; whether overlapping fleet assignments are allowed. 
  ...
  if (table_exists(fleet_tab, project) == FALSE) {
    ...
    stop("Table name does not exist in fishset_db. Check spelling or create a ",
         "fleet definition table with fleet_table().", call. = FALSE)
  }
  ...
  if (length(ovrlp) > 0) {
    ...
    if (overlap == FALSE) {
      ...
      stop("Fleet column not added. Set overlap = TRUE to allow overlapping ",
           "fleet assignments.", call. = FALSE)
    }
  }
