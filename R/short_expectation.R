...
short_expectations <- function(dat,
                               project,
                               catch,
                               price,
                               defineGroup,
  ...
  # TODO: Alt choice list should be arg in short_expectations (and other expectation funs)
  if (!exists("Alt")) {
...
#  }
  ...
  numData <- as.data.frame(numData)[z_ind, ] # (Alt.dataZoneTrue,:)
  spData <- choice[z_ind, ] # mapping to to the map file
  spNAN <- which(is.na(spData))
  ...
  numNAN <- which(is.nan(numData) == TRUE)
  ...
  C <- match(paste(temp[, 1], temp[, 2], sep = "*"), 
             paste(B[, 1], B[, 2], sep = "*")) # C = row ID of those unique items
  ...
  if (length(which(duplicated(df2$ID) == TRUE)) == 0) {
    ...
    warning("Selected groups and choice data results in only single observations. ", 
            "Cannot use lag time for chosen group and choice data. Setting lag ", 
            "time to 0.")
    
  } else if ((length(which(duplicated(df2$ID) == TRUE)) / length(df2$ID)) < .25) {
    
    lagTime <- 0
    warning(paste0("Selected groups and choice data results in ", 
                   length(which(duplicated(df2$ID) == FALSE)) / length(df2$ID) * 100,
  ...
  myfunc_ave <- function(x, y) {
    ...
    mean(df2[df2$tiData >= x - yearRange - timeRange & df2$tiData <= 
               x - yearRange & df2$ID == y, "lag.value"], na.rm = TRUE)
  }
  ...
  if (is.na(empty.catch) || is_value_empty(empty.catch)) {
    ...
    df2$ra[which(is.na(df2$ra))] <- unlist(lapply(df2$tiData[which(is.na(df2$ra))], myfunc_emp))
    # replaceValue <- NA
    
  } else if (empty.catch == 0) {
    
    df2$ra[which(is.na(df2$ra))] <- 0
    ...
    df2$ra[which(is.na(df2$ra))] <- unlist(lapply(df2$tiData[which(is.na(df2$ra))], myfunc_AC))
    ...
    df2$ra[which(is.na(df2$ra))] <- mapply(myfunc_GC, df2$tiData[which(is.na(df2$ra))], df2$ID[which(is.na(df2$ra))])
    # replaceValue <- aggregate(catchData, list(C), mean, na.rm = T)
    
  } else { # same results as NA or O 
...
if (calc.method == "simpleLag") {
  ...
  r <- nchar(sub("\\.[0-9]+", "", mean(as.matrix(newCatch), na.rm = TRUE))) # regexp(num2str(nanmax(nanmax(newCatch))),'\.','split')
  sscale <- 10^(r - 1)
