...
roll_catch <- function(dat,
  ...
  column_check(dataset, cols = c(catch, date, group_no_date, filter_by, facet_check))
  
  num <- qaqc_helper(dataset[catch], is.numeric)
  
  if (!all(num)) stop("Argument 'catch' must be numeric.", call. = FALSE)
  
  # date ----
  # convert date and/or sub_date to date class
  dataset[date] <- lapply(dataset[date], date_parser)
  
  # sub_date ---- 
  # check if sub_date is needed
  sub_date <- sub_date_check(sub_date = NULL, date, filter_date, group, facet_by)
  ...
  names_to <- function() {
    ...
    if (length(catch) == 1 & length(agg_grp) > 0) agg_grp
    else if (length(catch) > 1 & length(agg_grp) == 0) "species"
    else if (length(catch) > 1 & length(agg_grp) > 0) species_grp
  }
  ...
  if (!is.null(filter_date) & !is.null(sub_date)) {
    ...
    if (nrow(roll_tab) == 0) {
      ...
      stop("Filtered data table has zero rows. Check filter parameters.", 
           call. = FALSE)
    }
  }
  ...
  if (output %in% c("tab_plot", "plot")) {
    ...
    if (suppress) {
      ...
      rc_plot_c <- roll_catch_plot(replace_sup_code(check_out$table, 0), 
                                    catch, date, group_list, facet_by, fun, 
                                    k, conv, tran, format_lab, scale)
      save_plot(project, "roll_catch_confid", plot = rc_plot_c)
    }
  ...
  roll_catch_function$args <- list(dat, project, catch, date, group, combine, k, 
                                   fun, filter_date, date_value, filter_by, 
