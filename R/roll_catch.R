...
#'            
#' roll_catch(pollockMainDataTable, project = "pollock", 
...
roll_catch <- function(dat, project, catch, date, group = NULL, k = 10, 
  ...
  if (!is.null(group) | !is.null(facet_by)) {
    ...
    if (length(catch) == 1) {
      ...
      nm4 <- c(nm2, "species")
      agg <- tidyr::pivot_wider(agg, names_from = nm4, names_sep = "__", 
  ...
  if (!is.null(group)) {
    ...
    if (length(nm2) > 1 & length(catch) == 1) {
      ...
      rz <- tidyr::pivot_longer(rz, cols = -Index, names_to = nm2, values_to = "catch",
                                names_sep = "__")
      
    } else if (length(catch) > 1) {
      
      rz <- tidyr::pivot_longer(rz, cols = -Index, names_to = nm4, values_to = "catch",
                                names_sep = "__")
    }
