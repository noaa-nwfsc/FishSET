...
filter_outsample <- function(dat, project, mod.name, spatial_outsample = FALSE, zone.dat = NULL,
  ...
  if(spatial_outsample == FALSE){
    ...
    if(!is_empty(nonoverlap_zones)){
      if(isRunning()){
        showNotification(paste0(length(nonoverlap_zones), " zone(s) from model not present in out-of-sample data."), type = "warning", duration = 60)
      } else {
    ...
    if(flag == 0){
      if(isRunning()){
        showNotification("Filtered out-of-sample data saved", type = "message", duration = 60)
      } else {
        message("Filtered out-of-sample data saved.")
      }  
    } else if (flag == 1){
      if(isRunning()){
        showNotification("Save was unsuccessful. Check filter settings.", type = "error", duration = 60)
      } else {
