...
The FishSET GUI is a user-friendly interface that guides users through the steps necessary to run discrete choice models. The FishSET GUI requires no knowledge of coding or R as all steps to prepare and run discrete choice models can be run in the FishSET GUI apart from writing the report. The report template must be opened in the R console. 
...
We recommend the following steps to install FishSET:
 ...
 * Save plot and table buttons
    * Saves output to a user-specified location.
    * Only use if you want to save output to a location other than the FishSET package directory. Tables and plots are automatically saved to the FishSET `Output` folder when generated. 
 * Refresh data
    * Pulls the primary data table from the FishSET database.
    * To refresh to a specific saved version of the dataset, go to the `Upload data` tab.
    * Unsaved changes to the dataset will be lost when the data is refreshed.
 * Close app
