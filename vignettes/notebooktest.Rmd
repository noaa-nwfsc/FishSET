...
#Load main data set into fishset_db database
load_maindata(dat, over_write = TRUE, project= 'pcod', compare = FALSE, y = NULL)
#Loadd port data set into fishset_db database
...
The plotted haul locations appear to be in the correct location but some occur over land. We need to further check these. We may also want to further check for outliers.

```{r echo=TRUE}
outlier_plot('pcodMainDataTable', 'OFFICIAL_TOTAL_CATCH', dat.remove='5_95_quant', x.dist='lognormal')
```

The plot output in this example does not provide evidence that outliers should be removed.
...
#RUN MODEL FUNCTIONS
results <- discretefish_subroutine("pcod", initparams=c(2.5, -.1), optimOpt=c(1000,1.00e-08,1,1), func=logit_c, methodname="BFGS", mod.name='pcod_logitParamSet1', select.model=FALSE)
print(results)
```
