...
<!--# Create folder path to the FishSET folder. -->
...
Reset the the `scallopMod` project if it exists. Note: when running this
chunk you may get a pop-up asking to identify the location for a new
FishSET folder or an existing folder.

```{r}
# for running .Rmd, remove for vignette version
if (project_exists('scallopMod')) erase_project('scallopMod')
```
...
vars_to_keep <- c('TRIPID', 'PERMIT.y', 'DATE_TRIP', 'DDLON', 'DDLAT', 'ZoneID', 
                  'POUNDS', 'port_lon', 'port_lat', 'previous_namelsad',
...
Load each dataset into FishSET. Note: when running this chunk you may
get a pop-up asking to identify the location for a new FishSET folder or
an existing folder.
...
```{r}
zone_summary(scallopModMainDataTable, 
...
expected catch parameters and `V1` the travel distance parameter.

```{r}
model_params("scallopModModelOut", "scallopMod", output = 'print')
```
