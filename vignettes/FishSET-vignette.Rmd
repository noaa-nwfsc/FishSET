...
## Before you begin
We have included an RMarkdown Notebook file. This file takes you through to the basic steps in FishSET from data loading to modeling. We recommend opening this file as a tutorial.

### Notebooks 
In addition, we are recommending the use of either Jupyter Notebooks or RMarkdown Notebooks for reproducing workflow and producing a report that incorporates code, text, and output. Jupyter Notebooks can be used in both a standard R terminal and an RStudio terminal. RMarkdown Notebooks can only be used in an RStudio terminal. In an RStudio session open and begin working in a notebook environment by clicking *File -> New File -> R Notebook*. If asked to add or update packages, click yes. A new file should soon appear with `output: html_notebook` at the top of the file. 


```{r fig1, echo=FALSE, fig.cap="Opening a new R Notebook file", out.width = '1%'}
knitr::include_graphics('C:/Users/melanie.harsch/Pictures/Screenshots/OpeningNotebook.png')
```



At this point, you can begin adding text and code chunks. To output the Notebook file, you will need to first save the file. Then click the *preview* button to view the document in the viewer. If the document does not appear in the view, click the *gear* icon and make sure the *Preview in viewer pane* is checked. To publish the document, select one of the *knit* options in the *preview* dropdown or add lines similar to the one below to the first chunk in your notebook file. See [Chapter 3 of RMarkdown: The Definitive Guide]( https://bookdown.org/yihui/rmarkdown/notebook.html) for more information.
...
In the regular R terminal, you will need to the Jupyter notebook. Follow [these directions](https://www.datacamp.com/community/blog/jupyter-notebook-r?utm_source=adwords_ppc&utm_campaignid=1565261270&utm_adgroupid=67750485268&utm_device=c&utm_keyword=&utm_matchtype=b&utm_network=g&utm_adpostion=1t1&utm_creative=332661264371&utm_targetid=dsa-473406574235&utm_loc_interest_ms=&utm_loc_physical_ms=9033316&gclid=EAIaIQobChMIxLzM6JaK4gIVDy2GCh2Vowc7EAAYASAAEgLJUvD_BwE) to load and activate an IRKernel and begin working with R in a notebook environment. Please note, these directions are for the regular R terminal and will not work in an RStudio terminal.


## Storing data and project delineation
FishSET is designed for reproducibility. All data is stored in a SQL database called fishset_db and all function calls are logged in a log file. 
...
All FishSET function calls are saved in dated log files. These log files are useful for documenting which functions were run and the parameter choices. They are also useful for rerunning analyes with updated data. Note that new log files will be saved each day FishSET functions are run. However, the list of saved functions to be logged will not reset between days if the FishSET package is not closed between days. This allows work completed over multiple days to be saved in the same file. To reset the list of saved function calls run `log_reset()`. 

```{r figLog, echo=FALSE, , out.width = '1%'}
knitr::include_graphics('C:/Users/melanie.harsch/Pictures/Screenshots/LogFolder.png')
```

Saving tables and lists to the database requires a call to open a connection to the SQLite database, a call to write the table to the database, and a final call to close the connection to the database. These three calls are done within the function. For instance, the `load_maindata` function opens a data set, runs a set of checks on the data set, and then saves the data set to the fishset_db database. Embedded in the function are the lines of code that save the table to the database.
...
Function calls are saved in dated files in the log folder. The log files saves the function name, the parameter values, including input data, and output. It is important to use the naming convention for tables saved to the SQL database. For instance,  use `pcodMainDataTable` rather than ambiguous names such as *dat*. Using the database naming conventions allows for easier recognition of the data and project along with ability to rerun the function using the log file. Note that the list of function calls is reset when either the FishSET library is closed or the R session is closed. Thus, although a new log file will be created each day that FishSET functions are called, the new function calls will be added to the list of functions called and saved the previous day. This is useful if the users is working on a project over multiple days but wants to keep all function calls within a single file. To reset the function list saved to the log file, use `log_reset()`.
...
Data are saved in the SQLite database, fishset_db.
  ...
  Data can be imported, checked that required columns are defined, optionally compared against previously saved versions of the data, and then saved into the database using:
     ...
     load_aux      Additional data such as vessel characteristics and fishery season. 
     load_grid     Additional data that varies over two dimensions such as SST and wind speed.
 ...
 Model output files can be view in the fishset_db database using:
...
Several functions exist that allow users to modify or create vectors and matrices. These functions are useful to ensure date vectors are in the correct format, to generate rate variables (such as catch/hours fished), calculating more complex variables such as trip distance, and adding variance to confidential data.
  ...
  Trip level data and variables: 
     ...
     trip_length    Trip duration defined by trip start and end date or date-time variables. Can also create hauls per trip and cpue.
