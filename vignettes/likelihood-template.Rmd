...
Here we provide a template for developing a likelihood function that can be seamlessly integrated into the FishSET package. We request that users "fork" the FishSET GitHub repo to develop and test new likelihood functions and create a pull request when the function is ready to be integrated into the package. More information on "forking" repositories and the GitHub workflow can be found [here](https://git-scm.com/book/en/v2/GitHub-Contributing-to-a-Project).

Feel free to reach out to our team at [nmfs.fishset@noaa.gov](mailto:nmfs@fishset.gov) with any questions about developing your own likelihood function.
...
Give your likelihood function an informative name (replace `likelihood_function` in the code chunk above). 
...
|           to zones 2, 3, and 1 are 5, 10, 15, repectively.
\
...
- **mod.name**: Name of model, which is created in the `make_model_design()` function.
...
The Pull Request for your likelihood function should not change any other code in the FishSET package. If changes to the package outside of your function are necessary, please contact the FishSET development team ([nmfs.fishset@noaa.gov](mailto:nmfs@fishset.gov)) to solve any issues.