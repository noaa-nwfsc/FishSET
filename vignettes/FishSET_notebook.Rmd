...
Now if we look at the fishset_db database there should be six data tables: raw and working copies of the main data table, an information table that was created by the *load_maindata* function, and raw and working copies of the port data created by the *load_port* function.
...
```{r echo=TRUE}
FishSET:::select_vars('pcodMainDataTable', 'pcod')
```
...
Use the *temp_obs_table* to check for sparsity. Zones with sparse data can be filterd from the data by setting a minimum haul value (min.haul parameter) in the *create_alternative_choice* function.
```{r echo=TRUE}
