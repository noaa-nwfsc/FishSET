...
This vignette contains a short description the FishSET package. A more detailed manual is available at the FishSET **WEBSITE**. 

## Installing
We recommend using the following commands to install FishSET
First, install **devtools**.  This package is necessary to install and build FishSET.  
...
FishSET includes a worked example. This file takes you through to the basic steps in FishSET from data loading to modeling. We recommend opening this file as a tutorial.


## Storing data and project delineation
FishSET is designed for reproducibility. All data is stored in a SQLite database called fishset_db and all function calls are logged in a *Logs* folder. Output (plots, table, messages, notes) are saved in an *Output* folder.
The database, output folder, and log folder are created in the working directory when the FishSET package is first loaded. The output can be called into the report template. The saved function calls can be called to rerun the analysis with updated or new data. 

Function calls require a `project` argument in the function call. The `project` argument is important to distinguish output and log calls between analyses. 
...
### Data
There are four types of data files that can be imported into FishSET. All optional data files must be linkable to the primary data file.
...
`load_data(pollock, name="pollockMainDataTable20200101")` loads the primary data from project *pollock*. *name* is optional and is used to specify a specific dated version of the data table.  

####Reading
Read data tables into the working environment without saving to the FIshSET database using `read_dat()`. This function is necessary for spatial data files which cannot be saved to the FishSET database at this time. 

`read_dat()` calls the appropriate function to read in data.   
...
####Working in the FishSET database 
 Once data are saved in the fishset_db database, they can be accessed and viewed using:
     ...
     table_exists(x)	      Check whether table x exists in the FishSET database.
     table_remove(x)	      Remove a table from FishSET database.
