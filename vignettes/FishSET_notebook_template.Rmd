...
Steps: 
1) Pull in data. Example code provided below
```{r, echo=FALSE } 
#dat <- read.dat('path\to\dat', data.type='csv')
```

2) Need to select what data we want from the dataset
```{r, echo=FALSE } 
if(params$update=='TRUE'){
FishSET:::select_vars_gui(dat, 'test')
}
```
...
```{r, ehco=FALSE}
FishSET:::table_format('summary_table') 
```

```{r, out.width = '60%', echo=FALSE}
knitr::include_graphics(paste0(getwd(), '/inst/output/Test_Regression_plot.png'))
```
