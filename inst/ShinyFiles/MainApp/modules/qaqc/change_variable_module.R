# =================================================================================================
# File: change_variable_module.R
# Description: This module displays a table showing all the variables in the primary data table, 
#              the class of the variable, and the first value. The user can update the class type 
#              (numeric, factor, character, date) for any of the variables. Before saving, the 
#              user will get a preview of the first few values after the change and decide whether 
#              or not to continue.
#
...
variable_class_server <- function(id, rv_project_name, rv_data){
  moduleServer(id, function(input, output, session){
    ...
    rv_change_var_df <- reactiveVal(NULL) # Reactive value for updated df with new class types
    
    # Update dropdown with primary data variables
    observe({
      ...
      updateSelectInput(session, "change_class_var_input", 
    ...
    observeEvent(input$var_class_btn, {
      ...
      var_to_change <- input$change_class_var_input # Save as static copy of variable
      new_class <- input$change_class_input # Save as static copy of class type
      
      # Store the original column for the "before" preview
      original_column <- original_data[[var_to_change]]
      
      # Use tryCatch to handle conversion errors
    ...
    observeEvent(input$save_class_btn, {
      ...
      updateSelectInput(session, "change_class_input", selected = "")
    })
  })
}
...
variable_class_ui <- function(id){
  ...
  tagList(
    bslib::layout_column_wrap(
      fill = TRUE,
      width = 1/3,
      selectInput(ns("change_class_var_input"),
                  label = "Select variable:",
                  choices = NULL),
      selectInput(ns("change_class_input"), 
